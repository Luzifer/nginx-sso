...
func (m mfaDuo) ValidateMFA(res http.ResponseWriter, r *http.Request, user string, mfaCfgs []mfaConfig) error {
	...
	for _, c := range mfaCfgs {
		...
		duoCheck, _ := duo.Check()
		//Check if Duo API is up and successfully connected with API details
		if duoCheck.Stat != "OK"{
			continue
		}
...
// func (m mfaDuo) exec(c mfaConfig, user string, ip string) (bool) {
...
// 	}
// 	for key, values := range r.Form {
// 		if strings.HasSuffix(key, mfaLoginFieldName) && strings.HasPrefix(values[0] && len(values[0]) > 0) {
// 			keyInput = values[0]
// 		}
// 	}
